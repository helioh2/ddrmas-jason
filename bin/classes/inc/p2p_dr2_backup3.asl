...
+!p2p_dr(P,C0,C): locally(P,C) <-
		...
		.send(C0, tell, locally(P,C)).

+!p2p_dr(P,C0,C): not(locally(P,C)) & support_finished(P) & support_finished(~P) <-
...
+!p2p_dr(P,C0,C): not(locally(P,C)) & not(support_finished(~P)) <-
			+waiting_p2p_response(P,C0);
			.print("Calling support for negation. ",~P);
			!support1(~P,C).
...
+provable(B,K,SS): waiting_for_support_return(R,P,C,Bs,1) & .member(B,Bs) & C\==K <-
	+provable(B,C,[K]);
	-provable(B,K,SS);
	.print("Aqui 7 ",Bs);
	!decrease_waiting(R,P,C,Bs,1).
...
+~provable(B,K,SS): waiting_for_support_return(R,P,C,Bs,1) & .member(B,Bs) <-   //QUANDO VAI VIR ~provable?
	-waiting_for_support_return(R,P,C,Bs, 1);
...
+waiting_for_support_return(R,P,C,[],2): supportive_rule(R,C,P,_) <-
	...
	!check_support_finished(R,P,C).
	
	
+!check_support_finished(R,P,C): support_finished(P) <-
	.print("Aqui 11");
	.print("Negation: ",~P);
...
+!check_support_finished(R,P,C): not(support_finished(P)) <- true.
	...
	if (provable(P,C,SS)) {
		if (provable(~P,C,SSn)) {
			if (not(stronger(SSn,SS,T))) {
				.send(C0, tell, provable(P,C,SS));
				.print("Aqui 12 - PROVABLE");
			} else {
				.send(C0, tell, ~provable(P,C,SS));
				.print("Aqui 12 - ~PROVABLE");
			};
